...
def save(args, to_save, save_dir, epoch):
	for elem in to_save:
		...
		pred_seg = Segmentation(copy.deepcopy(elem['input']), elem['output'], elem['names'])
...
def train(args, train_loader, model, criterion, optimizer, writer, experiment, global_step, epoch):
	...
	for batch in train_loader:
		...
		target = batch['mask'].to(device=device, dtype=torch.float32) # mask_type = torch.float32 if n_classes == 1 else torch.long
	...
	with torch.no_grad():
		with tqdm(total=n_val+1, desc='Validation round', unit='batch', leave=False) as pbar:
			for batch in loader:
				...
				output = activation(output)  # BCE with logits implicitly applies sigmoid so left out from underlying model
...
def train_net(net,
	...
	with experiment.train():
		...
		train_split, val_split = random_split(dataset, [n_train, n_val])
		train_loader = DataLoader(train_split, batch_size=batch_size, shuffle=True, num_workers=1, drop_last=True)
		val_loader = DataLoader(val_split, batch_size=batch_size, shuffle=False, num_workers=1, drop_last=False)
...
if __name__ == '__main__':
	...
	dataset = FrameDataset(args.inputs, args.labels,
						   transform=Compose([
